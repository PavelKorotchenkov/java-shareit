...
class ItemServiceImplTest {
	...
	void add_whenNewItemAdded_thenReturnCorrectItem() {
		ItemCreateDto itemToSave = ItemCreateDto.builder()
				...
				.available(true)
				.build();
		Item itemEntity = ItemDtoMapper.toItemCreate(itemToSave);
	...
	void update_whenValidItemUpdateDtoProvidedAndUserIsOwner_thenItemIsUpdated() {
		...
		Item oldItem = Item.builder().id(itemId).name("name").description("description")
				.available(true).user(owner).build();
		...
		when(userService.getById(userId)).thenReturn(UserDtoMapper.toDto(owner));
	...
	void update_whenUserNotOwner_thenThrowAccessDeniedException() {
		...
		when(userService.getById(userId)).thenReturn(UserDtoMapper.toDto(notOwner));
		when(itemRepository.findById(itemId)).thenReturn(Optional.of(oldItem));
	...
	void update_whenMultipleFieldsChanged_thenAllFieldsAreUpdated() {
		...
		Item updatedItem = Item.builder().id(itemId).name(newName).description(newDescription)
				.available(newAvailability).user(owner).build();
	...
	void getById_whenValidItemIdAndUserId_thenReturnsItem() {
		...
		Item item = Item.builder().id(itemId).name("name").description("description")
				.available(true).user(UserDtoMapper.toUser(userDto)).build();

		when(userService.getById(userId)).thenReturn(userDto);
		when(itemRepository.findById(itemId)).thenReturn(Optional.of(item));
		when(bookingRepository.findTop1ByItemUserIdAndStartDateBeforeAndStatusIn(anyLong(), any(), any(), any())).thenReturn(null);
		when(bookingRepository.findTop1ByItemUserIdAndStartDateAfterAndStatusIn(anyLong(), any(), any(), any())).thenReturn(null);
	...
	void getItemById_whenUserIsOwner_thenReturnsItemWithBookings() {
		...
		Item item = Item.builder().id(itemId).name("Test Item").user(owner).build();
		Booking lastBooking = Booking.builder().id(1L).item(item).build();
		Booking nextBooking = Booking.builder().id(2L).item(item).build();
		when(bookingRepository.findTop1ByItemUserIdAndStartDateBeforeAndStatusIn(anyLong(), any(), any(), any()))
				.thenReturn(lastBooking);
		when(bookingRepository.findTop1ByItemUserIdAndStartDateAfterAndStatusIn(anyLong(), any(), any(), any()))
	...
	void getUserItems_whenUserHasNoItem_thenReturnsEmptyList() {
		...
		when(itemRepository.findByUserId(userId)).thenReturn(itemList);
		when(bookingRepository.findByItemIdAndEndDateBeforeOrderByEndDateDesc(any(), any(), any(Pageable.class)))
				.thenReturn(new PageImpl<>(List.of(), page, 1));
		when(bookingRepository.findByItemIdAndStartDateAfterOrderByStartDateAsc(any(), any(), any(Pageable.class)))
				.thenReturn(new PageImpl<>(List.of(), page, 1));

		List<ItemWithFullInfoDto> result = itemService.getUserItems(userId, page);
	...
	void getUserItems_whenUserHasMultipleItems_thenReturnsListOfItems() {
		...
		List<Item> itemList = Arrays.asList(
				Item.builder().id(1L).name("Item 1").user(user).build(),
				Item.builder().id(2L).name("Item 2").user(user).build(),
				Item.builder().id(3L).name("Item 3").user(user).build());
		when(userService.getById(userId)).thenReturn(UserDtoMapper.toDto(user));
		when(itemRepository.findByUserId(userId)).thenReturn(itemList);
		when(bookingRepository.findByItemIdAndEndDateBeforeOrderByEndDateDesc(any(), any(), any(Pageable.class)))
				.thenReturn(new PageImpl<>(List.of(), page, 1));
		when(bookingRepository.findByItemIdAndStartDateAfterOrderByStartDateAsc(any(), any(), any(Pageable.class)))
		...
		List<ItemWithFullInfoDto> actualItems = itemService.getUserItems(userId, page);
	...
	void searchBy_whenValidUserIdAndText_returnsMatchingItemsDto() {
		...
		ItemDto itemDto = ItemDto.builder().id(itemId).name("item").ownerId(userId).build();
		...
		void addComment_whenAuthorHadBooking_thenSaveCommentAndReturnResponseDto() {
			...
			Item item = Item.builder()
					.id(itemId).name("item").user(UserDtoMapper.toUser(author)).build();
			Booking booking = Booking.builder().id(1L).item(item).build();

			CommentRequestDto commentRequestDto = CommentRequestDto.builder()
					.authorId(userId).itemId(itemId).text(text).build();
			Comment comment = Comment.builder()
					.author(UserDtoMapper.toUser(author)).item(item).text(text).build();
